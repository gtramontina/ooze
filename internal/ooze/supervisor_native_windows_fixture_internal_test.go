...
func TestWindowsNativeSupervisorDrainsChildInNestedJob(t *testing.T) {
	...
	after := len(awaitWindowsFixtureFile(t, markerPath, 2*time.Second))
	if after <= before {
		t.Fatalf("nested-job child %d did not prove it could execute before root exit", child)
	}
	...
	stoppedAt := len(awaitWindowsFixtureFile(t, markerPath, 2*time.Second))
	time.Sleep(100 * time.Millisecond)
	if final := len(awaitWindowsFixtureFile(t, markerPath, 2*time.Second)); final != stoppedAt {
		t.Fatalf("nested-job child %d remained executable after drainage: marker %d -> %d", child, stoppedAt, final)
	}
}
